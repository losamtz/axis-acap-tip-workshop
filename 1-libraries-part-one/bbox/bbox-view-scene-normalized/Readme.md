...
Draw a fixed green rectangle with scene normalization. In this case we are using GMainLoop to keep the rendering permanent, and destroy it only when exiting the application.
...
```bash
docker build --tag bbox-view-scenen --build-arg ARCH=aarch64 .
```
```bash
docker cp $(docker create bbox-view-scenen):/opt/app ./build
```