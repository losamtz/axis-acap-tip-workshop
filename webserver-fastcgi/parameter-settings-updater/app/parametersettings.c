...
int main(int argc, char **argv)
{
    ...
    // Start FastCGI in another thread
    g_thread_new("fcgi_thread", (GThreadFunc)fcgi_run, NULL);

    // ret = fcgi_run(); // Start the FastCGI server
    // if (ret != EXIT_SUCCESS)
    // {
    //     syslog(LOG_ERR, "[webserver] - Failed to start FastCGI server");
    //     goto exit_param;
    // }
    // syslog(LOG_INFO, "[webserver] - FastCGI server started successfully");
